...
float MyTools::Time::Clock::GetFramerate () {
    return 1.0f / (__DELTA_TIME);
}

float MyTools::Time::Clock::GetDeltaTime () {
    return __DELTA_TIME * __TIME_SCALE;
}

float MyTools::Time::Clock::GetDeltaTimeUnscaled () {
    return __DELTA_TIME;
}

float MyTools::Time::Clock::GetTimeSinceStart () {
    return __TIME_SINCE_START;
}

float MyTools::Time::Clock::GetTimeScale () {
    return __TIME_SCALE;
}

void MyTools::Time::Clock::Scale (float p_coeff) {
    __TIME_SCALE *= p_coeff;
}

void MyTools::Time::Clock::SetTimeScale (float p_timeScale) {
    __TIME_SCALE = p_timeScale;
}

void MyTools::Time::Clock::Initialize () {
    __DELTA_TIME = 0.0f;
...
void MyTools::Time::Clock::Update () {
